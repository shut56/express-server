...
const { startProcess, auth, getUser } = config;
    ...
    switch (method) {
      case 'get': {
        return {
          ...
          nextUrl:
            '/api/v1/learn/api/<your-id>/login/5d9b8990-ab41-43d4-ae2a-75852ca6744f',
          nextMethod: 'POST',
          body: {
            login: '<your-login>',
      ...
      case 'post': {
        ...
        if (login === 'test@test.com' && password === 'test') {
          return {
            ...
            message: "Let's see how deep you can go",
            nextUrl:
              '/api/v1/learn/api/<your-id>/dob/bb3bead9-3161-490e-a04b-52b5029d88ea',
            nextMethod: 'Put',
            body: {
            ...
            'content-type': 'application/json',
            description:
              'Body should contain date of birth. if day or month has single digit you need to use 0. 07.08.2019 -> correct. 7.8.2019 - incorrect',
          };
        }
        return {
          status: 'error',
      ...
      case 'put': {
        ...
        if (dob[0].length === 2 && dob[1].length === 2 && dob[2].length === 4) {
          return {
            status: 'ok',
            message:
              'You have unlocked achievement new <<API Warrior Level 1>>',
            description:
              'Congratulation! You just have added your birth day to your account',
          };
        }
        return {
          status: 'error',
...
<p>URL: /api/v1/learn/api/<</>your-id>/token</p>
...
server.get('/api/v1/learn/api/:uid/token', (req, res) => {
...
server.post(
  '/api/v1/learn/api/:uid/login/5d9b8990-ab41-43d4-ae2a-75852ca6744f',
  (req, res) => {
    const response = getAcces(req.method.toLowerCase(), req);
    if (response.status === 'ok') {
      res.json(response);
...
server.put(
  '/api/v1/learn/api/:uid/dob/bb3bead9-3161-490e-a04b-52b5029d88ea',
  (req, res) => {
    const response = getAcces(req.method.toLowerCase(), req);
    if (response.status === 'ok') {
      res.json(response);
